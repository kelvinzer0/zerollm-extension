...
export function formatMessagesToPrompt(messages, tools = []) {
  ...
  if (hasTools) {
    ...
    toolDirective += "   Jika perlu restart atau menjalankan service di background, gunakan pola mandiri bawaan Linux (POSIX) tanpa blocking:\n";
    toolDirective += "   - Matikan proses lama seketika: 'pkill -9 -f \"<nama>\" 2>/dev/null; sleep 1'\n";
    toolDirective += "   - Muat PATH binary environment ($HOME/.bun/bin, dll.) dan cd ke direktori proyek\n";
    toolDirective += "   - Lepaskan I/O dan job terminal (bawaan Linux tanpa harus nohup): '</dev/null >/dev/null 2>&1 & disown' (atau alihkan stdout/stderr ke file log jika perlu dipantau)\n";
    toolDirective += "   Pola Standar: pkill -9 -f \"<nama_proses>\" 2>/dev/null; sleep 1; export PATH=\"$HOME/.bun/bin:$PATH\" && cd <dir_proyek> && <COMMAND> </dev/null >/dev/null 2>&1 & disown; echo \"<nama_service> started PID: $!\"\n";
    toolDirective += '   Contoh: <zerollm_tool_call name="exec">{"command": "pkill -9 -f \\"node server.js\\" 2>/dev/null; sleep 1; export PATH=\\"$HOME/.bun/bin:$PATH\\" && cd /app && node server.js </dev/null >/dev/null 2>&1 & disown; echo \\"App started PID: $!\\""}</zerollm_tool_call>\n';
  ...
  if (hasTools) {
    ...
    endGuidance += "   Untuk perintah shell: Gabungkan langkah terkait menggunakan '&&'. Untuk daemon/server lepaskan I/O dengan '</dev/null >/dev/null 2>&1 & disown' (bawaan Linux) agar shell tidak hang.\n";
  }
